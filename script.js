...
// --- TAB 1: DATABASE LOGIC DENGAN FITUR EDIT ---
...
function renderStudentTable() {
    ...
    students.forEach(s => {
        ...
        tr.innerHTML = `
            ...
            <td><span class="badge badge-${s.score}">${scoreLabel}</span></td>
            <td>
                <button class="btn btn-warning" style="padding: 5px 10px; font-size: 12px;" onclick="startEditStudent(${s.id})">✏️ Edit</button>
                <button class="btn btn-danger" style="padding: 5px 10px; font-size: 12px;" onclick="deleteStudent(${s.id})">🗑️ Hapus</button>
            </td>
        `;
...
// Spin Otomatis Durasi Random 2-10 Detik
...
// --- TAB 3: ADVANCE INSTANT SPIN LOGIC (GLOBAL BALANCED & ADIL) ---
...
function distributeJusticeGlobal(allStudents, groups) {
    ...
    // 1. Tentukan batas max anak A per kelompok agar terbagi merata
    ...
    // 3. Kompensasi (2+ Anak A wajib minimal 1 Anak C)
...
function downloadGroupReport(containerId, titleInputId) {
    ...
    if (!container || container.children.length === 0) {
        alert("Belum ada kelompok yang terbentuk untuk disimpan!");
        return;
    }
    ...
    headerDiv.style.borderBottom = '3px solid #4f46e5';
    headerDiv.style.marginBottom = '15px';
    headerDiv.style.borderRadius = '8px';
    headerDiv.innerHTML = `
        <h2 style="color: #4f46e5; margin-bottom: 5px; font-size: 22px;">${reportTitle}</h2>
        <p style="color: #64748b; font-size: 13px; font-weight: bold;">
